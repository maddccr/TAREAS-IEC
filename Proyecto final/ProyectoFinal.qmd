...
set.seed(123)
sims <- simulacion_n(200) 
#Metrica: N final
final_N <- sims[, 60] 
running_mean <- cumsum(final_N)/(1:length(final_N))

#Agregamos ademas medida de dispersion 
...
ggplot() +
  ...
  geom_line(data = df_spline_sub, aes(x = t, y = N_interp, group = sim, color = sim),    alpha = 0.7) +
...
ggplot(coef_long, aes(x = valor)) +
  geom_histogram(fill = viridis(20)[13]) +
  facet_wrap(~coef, scales = "free") +
  theme_minimal() +
  labs(x = "Valor del coeficiente", y = "Frecuencia",
       title = "Distribucion de los coeficiente de interpolacion") +
  theme(plot.title = element_text(size = 15, face = "bold", hjust = 0.5),
    legend.position = "right")
...
dist_vec <- as.numeric(distancias)
...
ggplot(df_dist) +
  geom_histogram(aes(x=distancia), bins = 30, fill=viridis(20)[2]) +
  labs(title = "Distancia entre simulaciones", x="Distancia euclidiana entre simulaciones" , y= "Distribucion")
