...
Puntos fijos de funciones con fixedpoint_show
...
# (a) cos(x), para x0=1,3 y 6 :
f_a<-function(x)cos(x)

fixedpoint_show(f_a,1)
fixedpoint_show(f_a,3) 
fixedpoint_show(f_a,6)
#en los 3 casos cos(x) converge aproximadamente a 0,739

# (b) exp(exp(-x)) para x0=2:
f_d<-function(x) exp(exp(-x))
fixedpoint_show(f_b,2)

# (c)  x -log(x) + exp(-x), para x0=2:
f_c<-function(x) x-log(x)+exp(-x)
fixedpoint_show(f_c,2)

#las funciones b y c convergen a 1.3098

# (d)  x + log(x) - exp(-x), para x0=2:
f_d<-function(x) x+log(x)-exp(-x)
fixedpoint_show(f_d,2) #diverge
```
