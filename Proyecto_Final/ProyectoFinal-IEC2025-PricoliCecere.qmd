...
# *Simulación del crecimiento poblacional logístico mediante Modelos Basados en Agentes (ABM)*
...
ggplot(df) + 
  geom_hline(yintercept  = 55, color = viridis(20)[13],
             linetype = "dashed", alpha = 0.8) +
  geom_hline(yintercept  = 58, color = viridis(20)[2], 
             linetype = "dashed", alpha = 0.8) +
  geom_line(aes(x = t, y = N, color = simu), linewidth = 0.5 ) +
  geom_line(aes(x = t, y = p*max(N), color = simu), 
            linewidth = 0.5) +
  geom_point(aes(x = t, y = N, color = simu, shape = "N"), size = 2.5) +
  geom_point(aes(x = t, y = p*max(N), color = simu, shape = "pB efectiva"),
             size = 2, stroke = 0.8, fill = "white") +
  labs(title = "Evolución en el tiempo de la población y la probabilidad efectiva de reproducción", subtitle = "ABM base",
       x = "Tiempo", 
       y = "Población N(t)\n Probabilidad de reproducción (%)", 
       color = "Parámetros") +
  scale_color_manual( values = viridis(20)[c(2,13)], name = "Simulación") +
  #scale_color_viridis(discrete = TRUE, option = "D") +
  scale_shape_manual(values = c("N" = 16, "pB efectiva" = 22),
    name = "Variable") +
  theme_minimal() +
...
sim_splines <- interpolar_sims(sim) #nSim x nPuntos (200)
```
...
t200 <- seq(1, iter, length.out = 200) #ajustar el escala de las spline
...
#Comparacion de una simulacion con su interpolacion
df_sim1 <- data.frame(
  t = 1:length(sim[1, ]),
  N = sim[1, ]
)

ggplot() + geom_line(data = df_spline |>  filter(sim == 1), size = 0.8,
    aes(x = t, y = N_interp, color = "Interpolación")) +
  
           geom_point(data = df_sim1, size = 1, alpha = 0.8,
    aes(x = t, y = N, color = "ABM Base")) +
  
    scale_color_manual(values = viridis(20)[c(2, 13)],name = "") +
    labs(title = "Original vs Interpolación", 
         subtitle = "para una simulación del ABM base",
...
#Para menos ruido grafico se eligen de forma aleatoria un tercio de las simualciones
set.seed(1234)
subset_idx <- sample(1:nrow(sim_splines), 20)
df_spline_sub <- df_spline |> filter(sim %in% subset_idx)

p15 <- ggplot() +
  geom_line(data = df_spline_sub, aes(x = t, y = N_interp, group = sim), 
            color=viridis(10)[5], alpha = 0.5) +
  geom_line(data = df_EDO, aes(x = t, y = N_EDO),
            color = "black", size = 0.7) +
  #scale_color_viridis_c(end = 0.8) +
  coord_cartesian(xlim = c(0, 15)) +
  labs(title = "Simulaciones interpoladas vs. EDO (t = 0-15)",
       subtitle = "ABM base",
       x = "Tiempo",y = "Población N(t)") +
  theme_minimal() +
  theme(plot.title = element_text(size = 15, face = "bold", hjust = 0.5),
...
p50 <- ggplot() +
  ...
  geom_line(data = df_EDO, aes(x = t, y = N_EDO),
            color = "black", size = 0.7) +
  #scale_color_viridis_c(end = 0.8) +
  coord_cartesian(xlim = c(15, iter), ylim = c(85, 100)) +
  labs(title = "Simulaciones interpoladas vs. EDO (t = 15-50)",
       subtitle = "ABM base",
       x = "Tiempo",y = "Población N(t)") +
  theme_minimal() +
  theme(plot.title = element_text(size = 15, face = "bold", hjust = 0.5),
...
coefs_total <- function(coeficientes) {
...
coef_globales <- coefs_total(coeficientes)
...
ggplot(coef_long, aes(x = valor)) +
  geom_histogram(fill = viridis(20)[13], color = viridis(20)[13], alpha = 0.5) +
...
extraer_coef<-function(letra) {
  
  t <- nrow(coeficientes[[1]])   # 50
  n_sims <- length(coeficientes) # 60
  # tomo la columna
  vals <- coef_globales[, letra]
  # convierto a matriz de 60 filas × 50 columnas
  matriz <- matrix(vals, nrow = n_sims, ncol = t, byrow = TRUE)#lleno la mtriz fila por fila con valores de B de a 50
  #nombres
  colnames(matriz) <- paste0(letra, 1:t)
  rownames(matriz) <- paste0("sim", 1:n_sims)
  
  return(matriz)
}

matrizA <- extraer_coef('A')
matrizB <- extraer_coef('B')

#paso a df
matrizB_df <- data.frame(matrizB)
matrizB_df <-mutate(matrizB_df, sim = rownames(matrizB_df))

matrizA_df <- data.frame(matrizA)
matrizA_df <-mutate(matrizA_df, sim = rownames(matrizA_df))

#paso a long para geom_point
matrizB_df_long <- pivot_longer( matrizB_df,
    cols = starts_with("B"),
    names_to = "coef_num",
    values_to = "valor"
  )
matrizB_df_long <- mutate(matrizB_df_long, coef_num = as.numeric(str_remove(coef_num, "B")))

matrizA_df_long <- pivot_longer( matrizA_df,
    cols = starts_with("A"),
    names_to = "coef_num",
    values_to = "valor"
  )
matrizA_df_long <- mutate(matrizA_df_long, coef_num = as.numeric(str_remove(coef_num, "A")))

ggplot(matrizB_df_long, aes(x = coef_num, y = valor)) +
  geom_point(alpha = 0.3, color=viridis(20)[13]) +
  theme_minimal() +
  labs(title = "Coeficientes B",
       x = "Índice del coeficiente",
       y = "Valor") +
  theme(plot.title = element_text(size = 15, face = "bold", hjust = 0.5))

ggplot(matrizA_df_long, aes(x = coef_num, y = valor)) +
  geom_point(alpha = 0.3, color=viridis(20)[2]) +
  theme_minimal() +
  labs(title = "Coeficientes A",
       x = "Índice del coeficiente",
       y = "Valor") +
  theme(plot.title = element_text(size = 15, face = "bold", hjust = 0.5))
```
...
ABM_ext <- function(K, iter, p_death, p_birth, r = 1, plot = FALSE) {
  ...
  df_N <- data.frame(N = N, p = p_b, t = 1:iter)
  
return(list(N = N, plot = timestamps_plot, df_N = df_N))
...
K <- 500
set.seed(1234)
ABM_3 <- ABM_ext(K, iter, 0.1, 0.9, plot = TRUE)
...
#GRAFICAR CURVA
set.seed(1234)
ABM_4 <- ABM_ext(K, iter, 0.3, 0.8)

df <- bind_rows(
  ABM_3$df_N |>  mutate(simu = "pD=0.1, pB=0.9"),
  ABM_4$df_N |>  mutate(simu = "pD=0.3, pB=0.8")
)

ggplot(df) + geom_point(aes(x = t, y = N, color = simu)) +
  #geom_point(aes(x = t, y = p*max(N), color = simu, shape = "pB efectiva"),
             #size = 0.8) +
  labs(title = "Evolución en el tiempo de la población", 
       subtitle = "ABM extendido",
       x = "Tiempo", 
       y = "Población N(t)", 
       color = "Parámetros") +
  scale_color_manual( values = viridis(20)[c(2,13)], name = "Simulación") +
...
simulacion_n_ext <- function(n, pD, pB){
  sims <- matrix(NA, nrow = n, ncol = iter) 
  for (i in 1:n) {
    res <- ABM_ext(500, iter, pD, pB)
    sims [i, ] <- res$N
  }
  return(sims)
}

set.seed(1234) #estaria bueno paralelizar esto tbm pero bueno
sims_3 <- simulacion_n_ext(200, 0.1, 0.9) 
df_3 <- analisis_conver(sims_3)
df_3$sim <- "pD=0.1, pB=0.9"
mu_3 <- mean(sims_3[, iter])

set.seed(1234)
sims_4 <- simulacion_n_ext(200, 0.3, 0.9) 
df_4 <- analisis_conver(sims_4)
df_4$sim <- "pD=0.3, pB=0.9"
mu_4 <- mean(sims_4[, iter])

#df_ambas_ext <- rbind(df_3, df_4)
```

```{r}

p3 <- ggplot(df_3, aes(x = replicas, y = mean)) +
  #"banda" de confianza asumiendo aproximacion normal 
  geom_ribbon(aes(ymin = mean - 1.96*se, 
                  ymax = mean + 1.96*se), 
              alpha = 0.2, color = NA, fill = viridis(20)[13]) +
  geom_line(size = 0.8, color = viridis(20)[13]) +
  #promedio total
  geom_hline(yintercept = mu_3, linetype = "dashed") +

  labs(title = "Convergencia del N(t) en el tiempo final con pD = 0.1 pB = 0.9",
       subtitle = "ABM extendido",
       x= "Número de réplicas",
       y = "Promedio acumulado",
       caption = paste0("Promedio total: ", round(mu_3, 2))) +
  theme_minimal() +
  theme(plot.title = element_text(size = 15, face = "bold", hjust = 0.5),
        plot.subtitle = element_text(size = 13, hjust = 0.5, face = "italic"),
...
p4 <- ggplot(df_4, aes(x = replicas, y = mean)) +
  #"banda" de confianza asumiendo aproximacion normal 
  geom_ribbon(aes(ymin = mean - 1.96*se, 
                  ymax = mean + 1.96*se), 
              alpha = 0.2, color = NA, fill = viridis(20)[2]) +
  geom_line(size = 0.8, color = viridis(20)[2]) +
  #promedio total
  geom_hline(yintercept = mu_4, linetype = "dashed") +
  
  labs(title = "Convergencia del N(t) en el tiempo final con pD = 0.3 pB = 0.9",
       subtitle = "ABM extendido",
       x= "Número de réplicas",
       y = "Promedio acumulado",
       caption = paste0("Promedio total: ", round(mu_4, 2))) +
  theme_minimal() +
  theme(plot.title = element_text(size = 15, face = "bold", hjust = 0.5),
        plot.subtitle = element_text(size = 13, hjust = 0.5, face = "italic"),
        plot.caption = element_text(size = 10, hjust = 0.5, face = "italic"),
...
sens_paral <- function(K = 500, iter = 60, p_birth = 0.9, reps = 150, 
...
```{r, warning=FALSE}
#paciencia con este que tiene que hacer mucha cosa
p_death_vals <- seq(0.2, 0.35, by = 0.005)
set.seed(1234)
res <- sens_paral(p_death = p_death_vals, iter = iter)
...
df_long <- data.frame(
  ...
  se = sqrt(as.vector(res$var_N) / 150) #reps = 150
)
...
ggplot(df_long, aes(x = time, y = mean, color = p_death, group = p_death)) +
  geom_ribbon(aes(ymin = mean - 1.96*se, ymax = mean + 1.96*se,
                  fill = p_death), alpha = 0.2, color = NA) +
  geom_line(size = 0.8) +
  geom_hline(yintercept = N0, linetype = "dashed", size = 0.8) +
  
  #geom_text(inherit.aes = FALSE, aes(x = 1, y = N0, label = paste0("N0 = ", N0)),
  #color = "black", hjust = -0.1,vjust = -0.5,size = 3) +
  
  annotate("text", x = -1, y = N0 - 20, label = paste0("N incial = ", N0),
         color = "black", hjust = -0.1, vjust = -0.5, size = 3) +

  labs(color = expression(italic("p_death")),
       x = "Tiempo",
       y = "Población N(t) promedio",
       title = "Evolución en el tiempo de N(t) para distintos valores de p_death",
       subtitle = "ABM extendido") +
...
```{r}
df_EDO_long <- do.call(rbind, lapply(p_death_vals, function(pd) {
  
  r <- 0.9 - pd 
  N_EDO <- sols(iter, K, N0, r)
  
  data.frame(time = 1:iter, p_death = pd, N_EDO = N_EDO) }))


ggplot(df_EDO_long, aes(x = time, y = N_EDO, color = p_death, group = p_death)) +
  ...
  labs(color = expression(italic("p_death")),
       x = "Tiempo",
       y = "Población N(t)",
       title = "Solución de la EDO para distintos valores de p_death") +
  scale_color_viridis_c(option = "D") +
  scale_fill_viridis_c() +
...
*Si llegaste hasta aca gracias por mirar nos costo mucho :3*
